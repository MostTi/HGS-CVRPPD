...
function startHgs(inputFileObj, res, stellargraph) {

    var args = [];
    var solutionFileName = `solutions/newSolutionFile_${(new Date().toJSON().slice(0,19))}.json`.replace(/:/g,"-");

    args.push(`inputFiles/${inputFileObj.filename}`);
    ...
    algorithmProcess.on('close', code => {
        ...
        if (code == 0) {
            ...
            if (stellargraph) {
                console.log("Creating output file vor stellargraph")
...
function createStellargraphResponse(solutionData, inputFileObj) {
    ...
    var data = [];
    var result = solutionData.result;
...
function getEntry(id, source, target, nodes, distances) {
    ...
    entry.source = source;
    entry.target = target;
    entry.source_latitude = nodes.find(node => node.realId === source).latitude;
    entry.source_longitude = nodes.find(node => node.realId === source).longitude;
    entry.target_latitude = nodes.find(node => node.realId === target).latitude;
    entry.target_longitude = nodes.find(node => node.realId === target).longitude;
...
function createBusmanagementResponse(data) {
    ...
    for (var res of tempResult) {
        var tempRoutes = res.routes;
        var routeCounter = 0;
