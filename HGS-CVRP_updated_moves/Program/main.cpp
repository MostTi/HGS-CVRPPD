...
int main(int argc, char *argv[])
{
	try
	{
		...
		Params params(commandline.pathInstance, commandline.nbVeh, commandline.measure, commandline.pathSolution);
		...
		if (population.getBestFound() != NULL)
		{
			...
			population.getBestFound()->exportJsonFormat(commandline.pathSolution, -1);
			// population.exportSearchProgress(commandline.pathSolution + ".PG.csv", commandline.pathInstance, commandline.measure);
			if (commandline.pathBKS != "") population.exportBKS(commandline.pathBKS);
		}
		else {
			std::cout << "NO FEASIBLE SOLUTION -> NOT EXPORTING" << std::endl;
			exit(10);
		}
	}
