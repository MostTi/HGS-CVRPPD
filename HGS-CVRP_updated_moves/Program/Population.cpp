...
/*	Jonas Schrader
	- holds the two sub populations
	- contains efficient data structures to memorize the distances between solutions used in the diversity calculations
*/
...
void Population::generatePopulation()
{
	...
	{
		Individual * randomIndiv = new Individual(params);												// create 4 times mu new individuals
		...
		addIndividual(randomIndiv, true);																// add individual to population

		if (!randomIndiv->isFeasible && std::rand() % 2 == 0)  											// Repair half of the solutions in case of infeasibility
		{
			localSearch->run(randomIndiv, params->penaltyCapacity*10., params->penaltyDuration*10.);	// run with increased penalties
...
bool Population::addIndividual(const Individual * indiv, bool updateFeasible)
{
	...
	// if (indiv->isFeasible) std::cout << "DETECTED FEASIBLE SOLUTION" << std::endl;
	if (updateFeasible)
	{
	...
	// std::cout << "FEASIBLE SOLUTION Place: " << place << std::endl;
...
void Population::removeWorstBiasedFitness(SubPopulation & pop)
{
	...
	if (pop.size() <= 1) exit(23);		//throw std::string("Eliminating the best individual: this should not occur in HGS");
...
Individual * Population::getBinaryTournament ()
{
	...
	// std::cout << "sizes " << feasibleSubpopulation.size() << ' ' << infeasibleSubpopulation.size() << std::endl;
...
void Population::exportSearchProgress(std::string fileName, std::string instanceName, std::string measure)
{
	std::ofstream myfile(fileName);
	for (std::pair<clock_t, double> state : searchProgress)
		myfile << instanceName << ";" << measure << ";" << state.second << ";" << (double)state.first / (double)CLOCKS_PER_SEC << std::endl;
}
