...
Params::Params(std::string pathToInstance, int nbVeh, int seedRNG) : nbVehicles(nbVeh)
{
	...
	if (inputFile.is_open())
	{
		...
		{
			inputFile >> cli[i].custNum >> cli[i].coordX >> cli[i].coordY >> cli[i].deliveryNum >> cli[i].pickupNum;
			cli[i].custNum--;
			if (cli[i].deliveryNum > 0) cli[i].deliveryNum--;		// NEW
			if (cli[i].pickupNum > 0 ) cli[i].pickupNum--;			// NEW
